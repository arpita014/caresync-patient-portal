...
# Safe to run multiple times: doctor licence/email and patient email values are generated uniquely.
#
...
# Find a starting number above existing doctor records so this script can
# be safely run again without duplicate licence/email values.
cursor.execute("SELECT COUNT(*) FROM doctor")
doctor_offset = cursor.fetchone()[0]

for i in range(NUM_DOCTORS):
    name   = 'Dr. ' + fake.name()
    spec   = random.choice(SPECIALISATIONS)
    phone  = '9' + str(random.randint(100000000, 999999999))  # 10-digit Indian number
    email  = f'doctor{doctor_offset + i + 1}@caresync.in'
    lic    = f'MCI-{2000 + doctor_offset + i:04d}'
...
# Start patient email numbering after existing patient records.
cursor.execute("SELECT COUNT(*) FROM patient")
patient_offset = cursor.fetchone()[0]
...
for i in range(NUM_PATIENTS):
    ...
    email     = f'patient{patient_offset + i + 1}@example.com'
...
# Only use completed appointments that do NOT already have a bill.
# This makes the script safe to rerun after a previous partial run.
cursor.execute(
    '''
...
if len(completed_appointments) < NUM_BILLS:
    print(f'  Note: Only {len(completed_appointments)} unbilled completed appointments available.')
